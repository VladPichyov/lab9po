...
int main()
{
    ...
    int size;
    char* best_time; // Добавляем переменную для лучшего времени
    ...
    try
    {
        ...
        best_time = process(subscriptions, size);
        cout << "\n******************** Лучший результат ********************\n";
        cout << "Минимальное время: " << best_time << '\n';
        delete[] best_time; // Освобождаем память

        bool (*check_filter)(runner_subscription*) = NULL;
        ...
        if (!check_choose)
        {
            cerr << "Некорректный номер пункта" << '\n';
            return 1;
        }
